  ...
  <div class="container">
    ...
    <ul class="navbar-nav ms-auto gap-3 align-items-center">
        ...
        <li class="nav-item d-flex align-items-center">
            <form action="{{ route('masyarakat.logout') }}" method="POST" class="m-0">
                @csrf
...
<section id="welcome" class="hero py-5">
  <div class="container text-center">
    <h1>Halo, {{ session('masyarakat.nama') }}</h1>
...
<section id="pengaduan-saya" class="py-5 bg-light">
  <div class="container">
    ...
    @if (!isset($pengaduan) || $pengaduan->isEmpty())
    ...
    @else
      <div class="row g-4" id="pengaduan-list">
        @foreach ($pengaduan as $index => $p)
          ...
          <div class="col-md-6 pengaduan-item {{ $index >= 4 ? 'd-none' : '' }}">
            <div class="pengaduan-card h-100">
              <div class="pengaduan-card-header d-flex justify-content-between align-items-center {{ $statusClass }}">
                <div class="d-flex align-items-center gap-2 text-white">
                  <i class="bi {{ $statusIcon }}"></i>
                  <span class="fw-semibold">{{ ucfirst($p->status) }}</span>
                </div>
                <small>{{ \Carbon\Carbon::parse($p->tgl_pengaduan)->format('d-m-Y') }}</small>
              </div>
              <div class="pengaduan-card-body p-3">
                <p class="mb-3">{{ \Illuminate\Support\Str::limit($p->isi_laporan, 120) }}</p>
                <div class="text-end">
                  <a href="{{ route('masyarakat.pengaduan.show', $p->id_pengaduan) }}" class="btn btn-sm btn-outline-primary rounded-pill">Lihat Detail</a>
                </div>
              </div>
            </div>
          </div>
...
{{-- Inline script supaya langsung jalan tanpa dependensi ke layout --}}
<script>
    ...
    <div class="row g-4">
      ...
      <div class="col-md-6 col-lg-3">
        <div class="step-card h-100 text-center p-4 border-0 shadow-sm">
          <div class="step-icon bg-{{ $step['color'] }} bg-gradient text-white mx-auto mb-3">
            <i class="bi {{ $step['icon'] }}"></i>
          </div>
          <h5 class="fw-semibold mb-2">Langkah {{ $index + 1 }}</h5>
          <p class="text-muted small mb-0">{{ $step['title'] }}</p>
        </div>
      </div>
      @endforeach
    </div>
  ...
  <div class="container">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 mt-3">
      ...
      <small class="text-muted text-md-end">
        &copy; {{ date('Y') }} Pengaduan Masyarakat. Dibuat dengan Laravel.
      </small>
    </div>
  </div>
</footer>
